...
def Quote_Validation():
    ...
    paths_ckit_3 = base.range('G22').value  #Media CKIT
...
# 2.	QUOTE- MASTER DATA←→ SELL PRICING(WILLIAM)-Master    
    ...
    for i in range(0,max_row-1):
        pn = f"=IFERROR(VLOOKUP(B{i+2},tmp!B1:W80000,3,0),"
...
#------------------------------------------ 4.	QUOTE- PROGRAM MATRIX←→SELL PRICING(WILLIAM)-Master & CPC-Option Features & CPC-BUSA
    ...
    # normalized_columns
    normalized_columns = {col.replace(" ", "").lower(): col for col in df_quote_programmatrix.columns}
    search_column = 'AV Level 2'.replace(" ", "").lower()

    # Find original column
    original_column_name = normalized_columns[search_column]
        
    #add sum of price
    av_index = df_quote_programmatrix[df_quote_programmatrix.loc[:, original_column_name].isnull() == False].index.tolist()
...
#Program Matrix vs. AV Summary
    ...
    for i in range(0,max_row-1):
        sheet_av_summary.range(i+2,5).value = f"=IFERROR(VLOOKUP(C{i+2},'{sheet_name_program_matrix}'!D:O,12,0), \"Null\")"
        if sheet_av_summary.range(i+2,5).value != None:
            sheet_av_summary.range(i+2,5).value = f"=IFERROR(round(VLOOKUP(C{i+2},'{sheet_name_program_matrix}'!D:O,12,0),2), \"Null\")"
