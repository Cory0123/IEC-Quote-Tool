...
def cpct_checker():
    ...
    for file, name in zip(all_paths_cpct, all_names_cpct) :
        ...
        worksheet2 = cpc_traker[bu_sa]
        
        # Load 'BU SA' 
        df3 = pd.read_excel(xls, 'BU SA')
        # Rename A1 as "SA \nLevel 3" (temp work around for bug)
        df3.rename(columns={'A1': 'SA \nLevel 3'}, inplace=True)

        # Save
        for i in cpct_checkpath:
            with pd.ExcelWriter(cpct_checkpath, engine='xlsxwriter') as writer:
                df3.to_excel(writer, sheet_name='BU SA', index=False)
...
def quote_validation():
    ...
    for paths_quote in QuoteName_choose:
        ...
        # Rename all D1 name= "IMX price", in  case of 2 "Current" in header as keyword
        df_quote_masterdata.rename(columns={'D1': 'IMX price'}, inplace=True)
        # Rename all G1 name= "IMX price", in  case of 2 "Current" in header as keyword
        df_quote_masterdata.rename(columns={'G1': 'Current Month'}, inplace=True)
        # Rename Ckit header
        df_quote_ckit.rename(columns={'J1': 'Current price'}, inplace=True)
    ...
    writer = pd.ExcelWriter(dirName+'/Quote_Validation_'+datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')+'rev.xlsx', engine='xlsxwriter')
    ...
    #writer.save()
    writer.close()
    base.range('B19').value = dirName+'/Quote_Validation_'+datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')+'rev.xlsx'
...
#                 sheet_name_skusummary = i
        ...
        df_quote_avsummary.loc[df_quote_avsummary[new_effective_day].isnull(), new_effective_day] = 'NotAvailable'
        ...
        df_quote_skusummary.loc[df_quote_skusummary[new_effective_day].isnull(), new_effective_day] = 'NotAvalible'
...
def IMX_ExcelTool():
    ...
    def process_excel(self, file_path):
        try:
            ...
            # Find the "Master Data" sheet ignoring case and spaces
            for sheet_name in sheet_names:
                if sheet_name.strip().lower() == "masterdata":
                    master_data_sheet = wb[sheet_name]
                    break
            
            if master_data_sheet:
                # Modify D1 and G1 cells
                master_data_sheet['D1'] = "Current Month"
                master_data_sheet['G1'] = "Total Cost- IMX"
                
                # Create directory if not exists
                output_folder = os.path.join(os.path.dirname(file_path), "Quote_rev")
                os.makedirs(output_folder, exist_ok=True)
                
                # Construct new file path
                file_name = os.path.basename(file_path)
                new_file_path = os.path.join(output_folder, f"{os.path.splitext(file_name)[0]}_rev.xlsx")
...
## 2-1.function2-1: Let user chooses the Quote files ---End-----
        ...
        finish_code() #Let user user know the code is finished (pop up Done buttion.)(Please refer to function1-3)